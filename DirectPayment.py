...
NOTIFICATION_SECRET = 'CbJgZDJ13J7onxBlgDRaRjUE' # this is a fake secret, guys, - so, don't even try
...
def HTTPNotifications(request):
	...
	if request.method == "POST":
		request_expected_elements = {
			...
			OPERATION_ID:		'^([0-9]{1,})$',			#1234567
			AMOUNT:				'^([0-9]{1,}\.[0-9]{2})$',		#300.00
			CURRENCY:			'^(643)$',				#643
			DATETIME:			'^([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}[A-Z])$',\
			#2011-07-01T09:00:00Z
			SENDER:				'^([0-9A-Z]+)$',			#41001XXXXXXXX
			CODEPRO:			'^(false)$',				#false
			LABEL:				'^([0-9a-z]{1,})$',			#hjg314kjh24312769z2187t0ehio
			SHA1_HASH_YANDEX:	'^([0-9a-z]{1,})$',			#090a8e7ebb6982a7ad76f4c0f0fa5665d741aafa
		}
		...
		SHA1_HASH_OURS = input_correct[NOTIFICATION_TYPE] \
			+ '&' + \
		...
		sha1_hash_yandex = input_correct[SHA1_HASH_YANDEX]
